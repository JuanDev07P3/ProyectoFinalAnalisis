...
private void configurarModeloDeTabla() {
    ...
    modeloTablaProductos.addColumn("Stock"); // <-- Esta es la columna 4 (índice)
    ...
    if (tblProductos != null) {
        ...
        // 1. Definimos el índice de la columna "Stock".
        int columnaStockIndex = 4; 

        // 2. Creamos una instancia de nuestro renderer.
        LowStockRenderer stockRenderer = new LowStockRenderer(columnaStockIndex);

        // 3. Le decimos a CADA columna de la tabla que use nuestro renderer.
        //    Esto asegura que toda la fila se pinte del mismo color.
